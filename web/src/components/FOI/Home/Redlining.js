  ...
  (
    ...
    const getRedactionObj = (newRedaction, editAnnot, _resizeAnnot, isPolygonToolSelected) => {
      if (isPolygonToolSelected) {
        // find highest redaction
        let highestRedaction = docViewer.getToolMode().redactionAnnotations[0];
        for (const redaction of docViewer.getToolMode().redactionAnnotations) {
          if (redaction.getRect().y2 < highestRedaction.getRect().y2) {
            highestRedaction = redaction;
          }
        }
        return {
          obj: highestRedaction,
          names: [highestRedaction.Id]
        }
      }
    ...
    const saveRedaction = async (_resizeAnnot = {}) => {
      ...
      if (editAnnot || _resizeAnnot?.type === "redact") {
        ...
        for (const node of redactions) {
          ...
          let annotGroup = [];
          if (isPolygonToolSelected) {
            annotGroup = docViewer.getToolMode().redactionAnnotations
          } else {
        ...
        let annotationList = [];
        if (isPolygonToolSelected) {
          annotationList = docViewer.getToolMode().redactionAnnotations;
