...
import useSaveResponsePackage from "./CreateResponsePDF/useSaveResponsePackage2";
...
const Redlining2 = React.forwardRef(
  (
    {
      user,
      requestid,
    ...
    useEffect(() => {
      let initializeWebViewer = async () => {
        ...
        WebViewer(
          ...
          polyRedactTool.setStyles({StrokeColor: new Annotations.Color(255, 0, 0, 1)})
    ...
    const annotationChangedHandler = useCallback(
      (annotations, action, info) => {
        ...
        if (docViewer.getToolMode().name === "PolygonRedactCreateTool") {
          if (annotations[0].Subject !== "Free Text" && (action === "add" || action === "modify")) {
            return;
          }
        }
    ...
    const getRedactionObj = (newRedaction, editAnnot, _resizeAnnot, isPolygonToolSelected) => {
      if (isPolygonToolSelected) {
        return {
          obj: docViewer.getToolMode().boundary,
          names: [docViewer.getToolMode().boundary.Id]
        }
      }
    ...
    const saveRedaction = async (_resizeAnnot = {}) => {
      ...
      setSelectedPageFlagId(null);
      setSaveDisabled(true);
      let isPolygonToolSelected = docViewer.getToolMode().name === 'PolygonRedactCreateTool'
      // if (docViewer.getToolMode().name === 'PolygonRedactCreateTool') {
      //   // docViewer.getToolMode().redactionAnnotations = []
      //   // return;
      // }
      const exisitngAnnotations = annotManager.getAnnotationsList();
      let redactionObj = getRedactionObj(newRedaction, editAnnot, _resizeAnnot, isPolygonToolSelected);
      let astr = isPolygonToolSelected ? "" : parser.parseFromString(redactionObj.astr) ;
      ...
      if (editAnnot || _resizeAnnot?.type === "redact") {
        ...
        let redactions = isPolygonToolSelected ? [redactionObj.obj] : astr.getElementsByTagName("annots")[0].children;
        for (const node of redactions) {
          let annotationsToDelete = [];
          let redaction = isPolygonToolSelected ? node : annotManager.getAnnotationById(node.attributes.name);          
          ...
          let annotGroup = [];
          if (isPolygonToolSelected) {
            annotGroup = [docViewer.getToolMode().boundary]
          } else {
            annotGroup = [redaction]
          }
          await annotManager.groupAnnotations(annot, annotGroup);
        }
        let annotationList = [];
        if (isPolygonToolSelected) {
          annotationList = [docViewer.getToolMode().boundary];
          let displayedDoc = pageMappedDocs.stitchedPageLookup[annotationList[0].getPageNumber()];
          let individualPageNo = displayedDoc.page;
          for (const annotation of annotationList) {
            annotation.setCustomData("originalPageNo", `${individualPageNo - 1}`);
            annotation.setCustomData("docid", `${displayedDoc.docid}`);
            annotation.setCustomData("docversion", `${displayedDoc.docversion}`);
            annotation.setCustomData("isPolygonRedaction", `true`);
          }
          docViewer.getToolMode().redactionAnnotations = []
        } else {
        ...
        saveAnnotation(
          ...
          isPolygonToolSelected ? redactionObj.names[0] : ""
        );
    ...
    const getCoordinates = (_annot, _redaction, X) => {
      ...
      var rect;
      if (_redaction.getQuads) {
        let quad = _redaction.getQuads()[0];
        rect = new docInstance.Core.Math.Rect(quad.x1, quad.y3, quad.x2, quad.y2);
      } else {
        rect =  _redaction.getRect()
      }
      const doc = docViewer.getDocument();
      ...
      _annot.FontSize = Math.min(parseInt(_redaction.FontSize) || 9, 9) + "pt";
      ...
      if (docViewer.getToolMode().name === 'PolygonRedactCreateTool') {
        ...
        _annot.setRect(new docViewerMath.Rect(highestPoint.x, highestPoint.y, highestPoint.x + _annot.Width, highestPoint.y + _annot.Height));
      }
      _annot.NoMove = true;
      return _annot;
    };
    ...
    const cancelRedaction = () => {
      ...
      if (docViewer.getToolMode().name === 'PolygonRedactCreateTool') {
        annotManager.deleteAnnotation(docViewer.getToolMode().boundary)
        docViewer.getToolMode().boundary = null;
      }
      setEditAnnot(null);
    };
...
export default React.memo(Redlining2);
