...
const DocumentSelector = ({
    ...
    const filterFiles = (filters: Array<number>) => {
        if (filters?.length > 0) {
            setFilesForDisplay(filteredFiles.filter((file: any) =>
                file.pageFlag?.find((obj: any) => (filters.includes(obj.flagid)))));
        }
        else
                    ...
                    <div>
                        ...
                        <span>
                            {pageFlagList.map((item: any) =>
                                <span >
                                    <FontAwesomeIcon key={item.pageflagid} title={item.name} className={'filterIcons'}
                                        onClick={(event) => applyFilter(item.pageflagid, event)} id={item.pageflagid}
                                                        ...
                                                        (filterFlags.length > 0 ?
                                                            ((file.pageFlag && file.pageFlag?.find((obj: any) => obj.page === p + 1 && filterFlags?.includes(obj.flagid))) &&
                                                                <>
                                                                    <StyledTreeItem nodeId={`file${index}page${p + 1}`} key={p + 1} icon={<FontAwesomeIcon className='leftPanelIcons' icon={assignPageIcon(file.documentid, p + 1) as IconProp} size='1x' />}
                                                                        title={getFlagName(file, p + 1)} label={isConsult(file.consult, p + 1) ? `Page ${getPageNoOfStitchedDoc(file, p + 1, pageMappedDocs)} (${ministryOrgCode(p + 1, file.consult)})` : `Page ${getPageNoOfStitchedDoc(file, p + 1, pageMappedDocs)}`} onClick={(e) => selectTreeItem(file, p + 1, e)}
                                                            ...
                                                            (file.pageFlag && file.pageFlag?.find((obj: any) => obj.page === p + 1) ?
                                                                <StyledTreeItem nodeId={`file${index}page${p + 1}`} key={p + 1} icon={<FontAwesomeIcon className='leftPanelIcons' icon={assignPageIcon(file.documentid, p + 1) as IconProp} size='1x' />}
                                                                    ...
                                                                    onContextMenu={(e) => openContextMenu(file, p + 1, e)} />
                                                                :
                                                                <StyledTreeItem nodeId={`file${index}page${p + 1}`} key={p + 1} label={`Page ${getPageNoOfStitchedDoc(file, p + 1, pageMappedDocs)}`}
