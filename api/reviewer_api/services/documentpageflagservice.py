...
class documentpageflagservice:
    ...
    def removebookmark(self,requestid, redactionlayerid, userinfo):
        ...
        for entry in pageflags:
            ...
            DocumentPageflag.savepageflag(requestid, entry['documentid'],  entry['documentversion'], json.dumps(new_pageflag), json.dumps(userinfo), redactionlayerid)
    ...
    def bulksavepageflag(self, requestid, data, userinfo):
        ...
        for entry in data["documentpageflags"]:
            try:
                ...
                results.append({"documentid": entry["documentid"], "version": entry['version'], "redactionlayerid": entry['redactionlayerid'], "message": result.message})
            except Exception as ex:
                logging.error(ex)
                results.append({"documentid": entry["documentid"], "version": entry['version'], "redactionlayerid": entry['redactionlayerid'], "message": "Page flag is not saved"})
        return results  
    ...
    def removepageflag(self,requestid, documentid, version, page, redactionlayerid, userinfo):
        pageflags, _ = self.getdocumentpageflags(requestid, redactionlayerid, documentid, version)
        ...
        if withheldinfullobj is not None:
            ...
            DocumentPageflag.savepageflag(requestid, documentid, version, json.dumps(pageflags), json.dumps(userinfo), redactionlayerid)
