...
func PublishNow(msg OpenInfoMessage, db *sql.DB) {
	...
	// THE TWO UPDATESITEMAP FUNCTIONS ON LINE 185 AND 186 WILL CAUSE THIS ISSUE POSSIBLY. NEED TO TEST AND FIX IF SO
	// PUBLISH NOW SHOULD ONLY ADJUST THE SITEMAP FOR THE REQUEST RELATED TO THE PASSED IN MSG. SHOULD NOT LOOK AT ALL READILY PUBLISHABLE REQUESTS WHICH GetOIRecordsForPublishing IN UPDATESITEMAP FUNC DOES
	Publish(msg, db)
