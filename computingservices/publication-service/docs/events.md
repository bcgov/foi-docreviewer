...
### Unpublishing (OI & PD)
The unpublish flow removes public repository objects under one published prefix, removes the public URL from sitemap XML, and emits a completion event with operational details. OpenInfo uses `openinfo.unpublish.requested` / `openinfo.unpublish.completed`; Proactive Disclosure uses `proactivedisclosure.unpublish.requested` / `proactivedisclosure.unpublish.completed`.
...
- **Semantics:** Emitted only after artifact publication and sitemap writing both succeed.
- **Payload Fields:** `tenant_id`, `request_event_id`, `correlation_id`, `publication_id`, `public_url`, `html_key`, `sitemap_index_key`, `sitemap_page_key`, `sitemap_page_url`, `sitemap_result`, `sitemap_index_updated`.
...
```json
{
  ...
  "payload": {
    ...
    "public_url": "https://proactivedisclosure.example.gov.bc.ca/packages/PD-2024-56789.html",
    "html_key": "proactivedisclosure/PD-2024-56789.html",
    "sitemap_index_key": "proactivedisclosurepub/sitemap/sitemap_index.xml",
    "sitemap_page_key": "proactivedisclosurepub/sitemap/sitemap_pages_1.xml",
    "sitemap_page_url": "https://proactivedisclosure.example.gov.bc.ca/sitemap_pages_1.xml",
    "sitemap_result": "written",
    "sitemap_index_updated": true
  },
...
Used for generating and updating sitemaps for the Open Information and Proactive Disclosure portals. In the main runtime, publish handlers call sitemap writing inline and emit enriched `*.publish.completed` events after the sitemap write succeeds. Standalone `*.sitemapping.requested` consumers are not started by `app.RunEventFlow`; these event contracts remain for isolated compatibility and tests.

#### `openinfo.sitemapping.requested`
- **Producer:** Legacy or isolated sitemap workflow
- **Consumer:** Standalone sitemap adapter, not started by the main runtime
...
- **Producer:** Standalone sitemap adapter
