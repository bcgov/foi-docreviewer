...
def processmessage(message):
    ...
    try: 
        ...
        recordjobstatus(pdfstitchmessage=message,version=3,error=None,isziping=True,status="zippingstarted",finalmessage="",message="")    
        result = __zipfilesandupload(message,s3credentials)
        recordjobstatus(pdfstitchmessage=message,version=4,error=None,isziping=True,status="zippingcompleted",finalmessage="",message="") 
        if result.get("success"):
            logging.info("final document path = %s", result.get("documentpath"))
            savefinaldocumentpath(result, message.ministryrequestid, message.category, message.createdby)
            recordjobstatus(pdfstitchmessage=message,version=5,error=None,isziping=False,status="completed",finalmessage="",message="")        
        else:
            errormessage = "Error in uploading the final document %s", result.get("filename")
            logging.error(errormessage)
            recordjobstatus(pdfstitchmessage=message,version=5,error=None,isziping=True,status="error",finalmessage=message,message="Zipfiles upload return an error")  
    except(Exception) as ex:
        logging.error("error in writing the bytearray")
        logging.error(ex)
        recordjobstatus(pdfstitchmessage=message,version=5,error=None,isziping=False,status="error",finalmessage=message,message=traceback.format_exc())
        raise
            ...
            with tempfile.SpooledTemporaryFile() as tp:
                with zipfile.ZipFile(tp, 'w',zipfile.ZIP_DEFLATED, compresslevel=9, allowZip64=True) as zip:
                    ...
                    for fileobj in _jsonfiles:                        
                        ...
                        zip.writestr(filename, __getdocumentbytearray(fileobj, s3credentials))                        
                tp.seek(0)
