...
// Initialize and returns a new RedisStreamWriter
...
func SendNotification(message *models.OCRProducerMessage, errorFlag bool) {
	writer := NewRedisStreamWriter(rdb)
	writer.SendNotification(message, errorFlag)
}

// Sends a notification to the Redis stream
func (w *RedisStreamWriter) SendNotification(message *models.OCRProducerMessage, errorFlag bool) {
	...
	// XAdd - adds message to the Redis stream
